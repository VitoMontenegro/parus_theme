	...
	<div class="content--bus content--post shortcode-toures_on_post"><div class="content__tours tours-slider post_tours_slider" id="tours">
		...
		<?php foreach($all_posts as $item): ?>
			...
			<?php if(get_post_meta($item->ID, 'tickets', 1) || !get_field('id_crm', $item->ID)): ?>					
				...
				<?php endif ?>
				<div class="content__tour tour <?=$terms?>" data-crm-id="<?=get_field('id_crm', $item->ID)?>" data-sale="<?php echo $is_sale; ?>" <?php if($datacost) { echo 'data-cost="' . $datacost . '"'; } ?>  data-duration="<?php echo $durationclear; ?>" data-exlusive="<?=get_field('eksklyuziv', $item->ID)?>" data-popular="<?php echo $num; ?>"  data-stuff='[<?php echo $data_date; ?>]'<?php  if($arr_get_date && empty(array_intersect($arr_get_date, $check_arr))){ echo 'style="margin-right: 10px; display: none;"';} ?>>
					...
					<div><a href="<?=$url?>" class="tour__content">
					...
					<?php endif; ?>
					  <div class="tour__cost">
						...
						// 	echo get_field('p_shkolniki_sale', $item->ID) . $rub;
						// } elseif (get_field('p_doshkolniki_sale', $item->ID)) {
						// 	echo  "<span>от <span id='min_cost'>";
						// 	echo "<span class='old-price-front'>".get_field('p_doshkolniki', $item->ID)."</span>";
						// 	echo get_field('p_doshkolniki_sale', $item->ID) . $rub;
						// } elseif ( get_field('p_doshkolniki', $item->ID) ) {
						// 	echo  "<span>от <span id='min_cost'>";
						//     echo get_field('p_doshkolniki', $item->ID) . $rub;
						// }  else {
...
add_shortcode( 'tour_on_post', 'tour_on_post_func' );
function tour_on_post_func( $atts ) {
    $atts = shortcode_atts( array(
        'ids' => '',
        'tour_per_page' => $atts['tour_per_page'],
    ), $atts, 'toures_on_post' );
...
//			),
	...
	<div class="content--bus content--post shortcode-toures_on_post toures-count-<?echo count($all_posts);?><?php if(count($all_posts) == 1){echo' only-one-item';};?>">
        <div class="content__tours tours-slider">
		<?php $num = 0; ?>
		<?php foreach($all_posts as $item): ?>
			<?php
				$periodicity = (get_field('periodicity', $item->ID))?get_field('periodicity', $item->ID):'по запросу';
			...
			?>
				...
				<?php if ($num == 5): ?>
					<?php get_template_part('promo'); ?>
				<?php endif ?>
				<div class="content__tour tour <?=$terms?>" <?php  if($arr_get_date && empty(array_intersect($arr_get_date, $check_arr))){ echo 'style="margin-right: 10px; display: none;"';} ?>>
                    <button class="wish-btn content__tour__wish-btn<? if($_COOKIE["product"]){ if(in_array($item->ID,$_COOKIE["product"])){echo' is-active';}}?>" data-wp-id="<?php echo $item->ID; ?>"  data-title='<?php echo $item->post_title;?>'>
                        <i class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
                                <g clip-path="url(#clip0_2_41)">
					...
					<?php else: ?>
						<?php /* <div class="tour__tickets-left tour__tickets-space"></div> */?>
					<?php endif; ?>
					  <div class="tour__cost">
					  	<?php if ( !in_array('excursions-in-english', $terms_arr) ): ?>
					  		<?php
