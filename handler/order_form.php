...
{	
	...
	//$excursion_date = $_POST['date_and_time']; 
	unset($_POST['date']);

	/* тема/subject */
	$subject =  htmlspecialchars ($subject.$xcursion_date." ".$_POST['phone']." ".$_POST['email']." ".$_POST['name']." ".$_POST['title']);
	//$subject =  preg_replace('/[^a-zA-Zа-яА-Я0-9]/ui', '',$subject );
	/* сообщение */
	$message = '<html><head> <title>Заявка. Письмо с сайта parus-peterburg.ru</title></head><body><table>';
	$text = 'С сайта parus-peterburg.ru было отправлено сообщение:' . "\n";
	$text = 'Тема: Заказать обратный звонок:' . "\n";
